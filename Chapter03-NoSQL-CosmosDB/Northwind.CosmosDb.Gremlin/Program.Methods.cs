...
{
    // whether to use Azure Cosmos DB in the local emulator or not
    ...
    private static string account = "apps-services-net7-graph";
    private static string primaryKeyCloud = "wo8NlkkJOzJN3PNA4vom76NKqdBjlSm73whrW7D6hRFoVUQ2z4yiqmFtB7yC05eelXtslXw6HgWPACDbl99WPA==";
    private static string endpointUriCloud = $"https://{account}.documents.azure.com:443/";
    ...
    private static string hostCloud = $"{account}.gremlin.cosmos.azure.com";
    ...
    {
        int affected = 0; // number of vertices or edges affected
        double requestChargeTotal = 0.0; // cost of executing the script, measured in RUs
        ...
        try
        {
            ...
            ResultSet<dynamic> resultSet = await gremlinClient.SubmitAsync<dynamic>(script);

            affected = resultSet.Count;

            if (double.TryParse(resultSet.StatusAttributes[RequestChargeHeader].ToString(), out double requestCharge))
            {
    ...
    static async Task CreateCosmosGraphResources()
    {
        ...
        try
        {
            ...
            DatabaseResponse dbResponse = await client.CreateDatabaseIfNotExistsAsync(database, throughput: 400 /* RU/s */);
    ...
    static async Task CreateProductVertices()
    {
        ...
        try
        {
            ...
            {
                if (p.ProductId == 15) break;
                string getProduct = $"""
                                        g.V().hasLabel("product")
                                        .has("productId", {p.ProductId})
                                    """;

                (int Count, double Cost) found = await ExecuteGremlinScript(getProduct);

                totalCharge += found.Cost;

                if (found.Count == 0)
                {
                    WriteLine("Product {0} not found. Adding its vertex...", p.ProductName);
                    string addProduct = $"""
                                            g.addV("product")
                    ...
                    (int Count, double Cost) added = await ExecuteGremlinScript(addProduct);
                    totalCharge += added.Cost;

                    WriteLine("{0} vertex was added at a cost of {1} RUs.", added.Count, added.Cost);
                }
                else
                {
    ...
    static async Task CreateCustomerVertices()
    {
        ...
        try
        {
            ...
            foreach (Customer c in db.Customers)
            {
                ...
                (int Count, double Cost) found = await ExecuteGremlinScript(getCustomer);
                totalCharge += found.Cost;

                if (found.Count == 0)
                {
                    WriteLine("Customer {0} not found. Adding its vertex...", c.CompanyName);
                    ...
                    (int Count, double Cost) added = await ExecuteGremlinScript(addCustomer);
                    totalCharge += added.Cost;
